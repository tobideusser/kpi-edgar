...
def lookup_kpi(kpi_dict, entity, sentence):
    return (kpi_dict["kpi"][entity.get_words(sentence.words)[0].id_]["num"],
...
def _convert_relations_to_excel_notation(
        ...
        add_unrelated_entities: bool = False
) -> List[Optional[str]]:
    ...
    if relations:
        ...
        for relation in relations:
            ...
            if head_entity.type_ == "kpi" and tail_entity.type_ != "davon":
                ...
                updated_head_entity = obtain_updated_entity(head_entity.get_words(sentence.words),
                                                            head_entity.type_ + f'_{num}')
                updated_tail_entity = obtain_updated_entity(tail_entity.get_words(sentence.words),
                                                            tail_entity.type_ + f'_{num}')
                ...
                if len(davon_list) == 1:
                    ...
                    updated_head_entity = \
                        obtain_updated_entity(head_entity.get_words(sentence.words),
                                              head_entity.type_ + f'_{num}_{davon_index + 1}')
                    updated_tail_entity = obtain_updated_entity(tail_entity.get_words(sentence.words),
                                                                tail_entity.type_ + f'_{num}')
                ...
                if len(davon_list) == 1:
                    ...
                    updated_head_entity = obtain_updated_entity(head_entity.get_words(sentence.words),
                                                                head_entity.type_ + f'_{num}')
                    updated_tail_entity = \
                        obtain_updated_entity(tail_entity.get_words(sentence.words),
                                              tail_entity.type_ + f'_{num}_{davon_index + 1}')
                ...
                updated_head_entity = obtain_updated_entity(head_entity.get_words(sentence.words),
                                                            head_entity.type_ + f'_{1}')
                updated_tail_entity = obtain_updated_entity(tail_entity.get_words(sentence.words),
                                                            tail_entity.type_ + f'_{1}')
...
def _write_document_name(ws: Worksheet, document: Document, new_split_type: Optional[str] = None) -> Worksheet:
    ...
    if new_split_type:
        cells = [
            WriteOnlyCell(ws, value=document.id_),
            WriteOnlyCell(ws, value=new_split_type)
        ]
    else:
...
def main():
    ...
    predicted_corpus_dir = "/shared_with_lars_and_thiago/ali/edgar_data/data_tasks/DataTagging/000/"
    ...
    save_path = "/shared_with_lars_and_thiago/ali/edgar_data/"
        ...
        for segment in document.segments:
            if isinstance(segment, Paragraph):
                for sentence in segment.sentences:
                    ...
                    converted_relations_pred = _convert_relations_to_excel_notation(sentence=sentence,
                                                                                    relations=sentence.relations_pred,
                                                                                    type_="pred",
                                                                                    add_unrelated_entities=True)
                    converted_relations_anno = _convert_relations_to_excel_notation(sentence=sentence,
                                                                                    relations=sentence.relations_anno,
                                                                                    type_="anno",
                                                                                    add_unrelated_entities=True)
