    ...
    def __call__(self, batch: Tuple[Sentence, ...]) -> Dict[str, Any]:
        ...
        try:
            entities_anno_iobes_ids = torch.stack(
                [torch.nn.functional.pad(torch.tensor(sentence.entities_anno_iobes_ids),
                                         pad=(0, max_words - sentence.n_words), value=self.pad_token_id)
        ...
        return {
            ...
            'entities_anno': [[ent.to_dict() for ent in sentence.entities_anno] if sentence.entities_anno else []
                              for sentence in batch],
            'relations_anno': [[rel.to_dict() for rel in sentence.relations_anno] if sentence.relations_anno else []
                               for sentence in batch],
            'word2token_alignment_mask': torch.stack(
                ...
                [torch.nn.functional.pad(torch.tensor(sentence.word2token_alignment_mask),
                                         pad=(0, max_tokens - (sentence.num_tokens - 1), 0, max_words - sentence.n_words),
                                         value=self.pad_token_id)
                 for sentence in batch]),
            'token_ids': torch.stack(
                [torch.nn.functional.pad(torch.tensor(sentence.token_ids),
            ...
            'word2token_start_ids': torch.stack(
                [torch.nn.functional.pad(torch.tensor(sentence.word2token_start_ids),
                                         pad=(0, max_words - sentence.n_words),
                                         value=-1)
                 for sentence in batch]),
            'word2token_end_ids': torch.stack(
                [torch.nn.functional.pad(torch.tensor(sentence.word2token_end_ids),
                                         pad=(0, max_words - sentence.n_words),
                                         value=-1)
                 for sentence in batch]),
        }
