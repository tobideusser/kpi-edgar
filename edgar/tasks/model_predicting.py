...
class ModelPredicting(Task):
    ...
    def _predict_corpus(
        self, model: JointNERAndREModel, dataloader: DataLoader, evaluator: Optional[Evaluator] = None
    ) -> Tuple[Dict, Dict]:
        ...
        for sentence in tqdm(corpus.sentences, desc="Write predictions"):
            ...
            if sentence_preds:
                ...
                sentence.entities_pred = [
                    Entity.from_dict({"start": start, "end": end, "type_": type_, "score": score})
                    for score, ((start, end), type_) in zip(
                        sentence_preds["entities_score"], sentence_preds["entities_pred"].items()
                    )
                ]
    ...
    def run(
        ...
        try:
            max_position_embeddings = model.encoder.encoder.config.max_position_embeddings
        ...
        if self.split_types:
            sentences = [
                ...
                if sentence.split_type in self.split_types and len(sentence.token_ids) <= max_position_embeddings
            ]
        else:
            sentences = [
                sentence for sentence in corpus.sentences if len(sentence.token_ids) <= max_position_embeddings
            ]
        # TODO change back
        ...
        dataloader = DataLoader(
            dataset=dataset, collate_fn=batch_collator, shuffle=False, batch_size=self.batch_size, drop_last=False
        )
