...
class ModelTraining(Task):
    ...
    def __init__(
        ...
        if self.type_ in ["pretrained_decoder", "pretrained_full"] and not self.path_pretrained_model:
            raise AttributeError(
                "If 'type_' is of a pretrained variant, a path to the pretrained model hast to be "
    ...
    def _create_torch_dataloaders(
        self, datasets: Dict[str, KPIRelationDataset], batch_collator: BatchCollator
    ) -> Dict[str, DataLoader]:
        dataloaders = {}
        set_seeds()
        for split_type, split_dataset in datasets.items():
            ...
            dataloaders[split_type] = DataLoader(
                dataset=split_dataset, collate_fn=batch_collator, shuffle=shuffle, **self.dataloader_params
            )
        return dataloaders
    ...
    def run(self, corpus_tokenized: Union[Dict, Corpus], labels: Dict, sub_word_tokenizer: PreTrainedTokenizerFast):
        ...
        if self.model_params["encoder_params"]["encoder_type_"] == "tfidf":
            ...
            if os.path.exists(path_to_tfIdf):
                ...
                vectorizer = TfidfVectorizer(
                    max_features=emb_dim, tokenizer=dummy_fun, preprocessor=dummy_fun, token_pattern=None
                )
        ...
        model = JointNERAndREModel.from_config(tokenizer=sub_word_tokenizer, labels=labels, **self.model_params).to(
            "cpu"
        )
        if self.type_ == "pretrained_full":
            ...
            pretrained_model = JointNERAndREModel.from_config(
                tokenizer=pretrained_sub_word_tokenizer, labels=labels, **pretrained_model_params
            ).to("cpu")
        ...
        optimizer = Optimizer.from_config(params=model.parameters(), **self.optimizer_params)
        ...
        trainer = Trainer(
            ...
            evaluator=evaluator,
            valid_dataloader=dataloaders.get("valid"),
            lr_scheduler=lr_scheduler,
            train_logger=train_logger,
            checkpointer=checkpointer,
            unique_config=self.unique_config,
            **self.trainer_params,
        )
