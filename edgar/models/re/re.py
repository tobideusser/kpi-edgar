...
from edgar import ALLOWED_RELATIONS, ALLOWED_1_TO_N_ENTITIES, ALLOWED_N_TO_1_ENTITIES
...
class REDecoder(nn.Module):
    ...
    def forward(self, batch: Dict):
        ...
        batch["relations_pred"] = pair2pred(
            batch=batch,
            vocab=self.labels,
...
def pair2pred(batch, vocab, remove_overlapping_relations: bool = True, table_to_text_mode: bool = False):
        ...
        for (head, tail), pred, score, representation in zip(
            ...
            for i in pred.nonzero():
                ...
                if tuple(head) in pred_entities[b] and tuple(tail) in pred_entities[b]:
                    ...
                    if table_to_text_mode:
                        rel["pair_representation"] = representation.detach().cpu()
