...
class SentenceEncoder(Encoder):
    ...
    def _init_encoder(type_: str, **kwargs_encoder):
        try:
            encoder_config, model_kwargs = AutoConfig.from_pretrained(
                type_, **kwargs_encoder, return_unused_kwargs=True
            )
        except OSError:
    ...
    def forward(self, batch: Dict) -> Dict:
        ...
        if self.word_pooling:
            batch["word_embeddings"] = token2word_embedding(
                data=batch, pooling=self.word_pooling, pooling_layer=self.pooling_layer
            )
        else:
