...
class JointDecoder(nn.Module):
    ...
    def forward(self, batch: Dict):
        ...
        ner_loss = ner_loss if ner_loss is not None else torch.tensor(0.0).to(get_device())
        re_loss = re_loss if re_loss is not None else torch.tensor(0.0).to(get_device())
